...
model_path = "denoising/drunet_LMMO_color_2024/models/7000_G.pth"
n_channels = 3
...
class PnP_ADMM(nn.Module):
    ...
    def forward(self, kernel, initial_uv, f, clean, sigma=25.5, lamb=690, sigma2=1.0, denoisor_sigma=25, irl1_iter_num=10, eps=1e-5): 
        ...
        for k in range(self.nb):
            ...
            u = run_model(t/255,denoisor_sigma) * 255 * 0.5 + 0.5*t
...
def search_args():
    ...
    # kernel 01, 
    search_range[0.04] = [1.3] # 25.5818, 0.6634
    # kernel 02, 
    # search_range[0.04] = [1.3] # 25.4478, 0.6639
    # kernel 03, 
    # search_range[0.04] = [1.3] # 26.2097, 0.6977
    # kernel 04, 
    # search_range[0.04] = [1.3] # 25.2279, 0.6491
    # kernel 05,
    # search_range[0.04] = [1.3] # 27.0773, 0.7303
    # kernel 06,
    # search_range[0.04] = [1.3] # 26.6894, 0.7134
    # kernel 07,
    # search_range[0.04] = [1.3] # 26.2344, 0.6989
    # kernel 08,
    # search_range[0.04] = [1.3] # 25.7587, 0.6803

    kernel_fp = '/home/dlwei/Documents/pnp_jacobian/kernels/kevin_8.png'
    # kevin's 8 kernels, CBSD68, 17.85 noise.
    # kernel 01, 
    search_range[0.05] = [1.1] # 24.8790, 0.6150
    # kernel 02, 
    # search_range[0.05] = [1.1] # 24.7962, 0.6214
    # kernel 03, 
    # search_range[0.05] = [1.1] # 25.5837, 0.6605
    # kernel 04, 
    # search_range[0.05] = [1.1] # 24.5764, 0.6070
    # kernel 05,
    # search_range[0.05] = [1.1] # 26.2294, 0.6822
    # kernel 06,
    # search_range[0.05] = [1.1] # 25.8268, 0.6627
    # kernel 07,
    # search_range[0.05] = [1.1] # 25.4842, 0.6538
    # kernel 08,
    # search_range[0.05] = [1.1] # 25.0439, 0.6367
...
# plot_psnr(0.07, 2.4, 17.85) # old


# # PnPI-FBS 0037.png, kernel 2. new

# a = 0.3, b = 0.15, iter = 50.
# plot_psnr(0.04, 2.5, 12.75) # 24.03, without 0.5D+0.5I

# a = 0.2, b = 0.1, iter = 100.
# plot_psnr(0.03, 1.2, 12.75) # 24.19, with 0.5D+0.5I. Now 0.5D+0.5I is non-expansive for k <= 0.5, thus lambda<=2.
# plot_psnr(0.04, 1.5, 12.75) # 24.25, with 0.5D+0.5I. Now 0.5D+0.5I is non-expansive for k <= 0.5, thus lambda<=2.
# plot_psnr(0.05, 2.0, 12.75) # 24.25, with 0.5D+0.5I. Now 0.5D+0.5I is non-expansive for k <= 0.5, thus lambda<=2.

# a = 0.1, b = 0.05, iter = 100.
# plot_psnr(0.04, 1.5, 12.75) # 24.31, with 0.5D+0.5I. Now 0.5D+0.5I is non-expansive for k <= 0.5, thus lambda<=2.
# plot_psnr(0.05, 2.0, 12.75) # 24.28, with 0.5D+0.5I. Now 0.5D+0.5I is non-expansive for k <= 0.5, thus lambda<=2.

# plot_psnr(0.05, 1.5, 17.85) # 23.42, with 0.5D+0.5I. Now 0.5D+0.5I is non-expansive for k <= 0.5, thus lambda<=2.
# plot_psnr(0.05, 1.3, 17.85) # 23.40, with 0.5D+0.5I. Now 0.5D+0.5I is non-expansive for k <= 0.5, thus lambda<=2.
# plot_psnr(0.05, 1.7, 17.85) # 23.30, with 0.5D+0.5I. Now 0.5D+0.5I is non-expansive for k <= 0.5, thus lambda<=2.
# plot_psnr(0.06, 1.6, 17.85) # 23.36, with 0.5D+0.5I. Now 0.5D+0.5I is non-expansive for k <= 0.5, thus lambda<=2.

# a = 0.1, b = 0.05, iter = 100.
# plot_psnr(0.06, 1.5, 12.75) # 23.97, with 0.3D+0.7I. Now 0.3D+0.7I is non-expansive for k <= 0.7, thus lambda<=2.
