...
class PnP_ADMM(nn.Module):
    ...
    def forward(self, kernel, initial_uv, f, clean, sigma=25.5, lamb=690, sigma2=1.0, denoisor_sigma=25, irl1_iter_num=10, eps=1e-5): 
        ...
        for k in range(self.nb):
            ...
            noise = torch.randn(u.size()).mul_(denoisor_sigma )
            noise = noise.type(torch.cuda.FloatTensor)

            v = (1-beta)*u + beta*  temp_ + beta*noise*step*255
            ...
            # vv = v - beta*noise*step*255
            # tt = vv/255
            # tt = tt.type(torch.cuda.FloatTensor)
            # ww_= run_model(tt,d) * 255
            # # ww_= w_ # for a fast calculation......
            # record_temp = abs_k*deblur.fftn(vv) - fft_kH*deblur.fftn(f)
            # record = step*(vv - ww_) + lamb_ * torch.real(deblur.ifftn(record_temp))
            # record = record / lamb_
            ...
            u = (1-alpha)*u + alpha*  temp_ + alpha*noise*step*255
...
def search_args():
    ...
    # level = 0.1, lambda = [3.1,3.2,3.3,3.4,3.5,3.6,3.7,3.8,3.9,4.0]
    # kernel 01, 
    search_range[0.1] = [3.4] # 27.3083, 0.7583
    # kernel 02, 
    # search_range[0.1] = [3.5] # 27.1031, 0.7498
    # kernel 03, 
    # search_range[0.1] = [3.9] # 27.5660, 0.7685
    # kernel 04, 
    # search_range[0.1] = [3.6] # 26.8959, 0.7422
    # kernel 05,
    # search_range[0.1] = [3.7] # 28.4926, 0.8028
    # kernel 06,
    # search_range[0.1] = [3.5] # 28.2327, 0.7946
    # kernel 07,
    # search_range[0.1] = [4.0] # 27.6470, 0.7737
    # kernel 08,
    # search_range[0.1] = [3.9] # 27.2215, 0.7583

    kernel_fp = '/home/dlwei/Documents/pnp_jacobian/kernels/kevin_8.png'
    # kevin's 8 kernels, CBSD68, 17.85 noise, 
    # level = 0.15, lambda = [4.1,4.2,4.3,4.4,4.5,4.6,4.7,4.8,4.9,5.0]
    # kernel 01, 
    search_range[0.15] = [4.1] # 26.3202, 0.7164
    # kernel 02, 
    # search_range[0.15] = [4.2] # 26.1653, 0.7107
    # kernel 03,  
    # search_range[0.15] = [4.4] # 26.6795, 0.7290
    # kernel 04, 
    # search_range[0.15] = [4.3] # 25.9671, 0.7029
    # kernel 05,
    # search_range[0.15] = [4.5] # 27.4601, 0.7633
    # kernel 06,
    # search_range[0.15] = [4.3] # 27.1792, 0.7546
    # kernel 07,
    # search_range[0.15] = [4.8] # 26.7195, 0.7358
    # kernel 08,
    # search_range[0.15] = [4.5] # 26.3082, 0.7181
...
# REDI-Prox 0037.png, kernel 2. step = 0.2, noise ratio = 255, output = w_, fine tuned
plot_psnr(0.1, 2.7, 12.75) # 24.83, chosen

# REDI-Prox 0037.png, kernel 2. step = 0.2, noise ratio = 255, output = w_, fine tuned
# plot_psnr(0.15, 3.5, 17.85) # 24.00
