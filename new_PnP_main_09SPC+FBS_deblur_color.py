...
class PnP_ADMM(nn.Module):
    ...
    def forward(self, kernel, initial_uv, f, clean, sigma=25.5, lamb=690, sigma2=1.0, denoisor_sigma=25, irl1_iter_num=10, eps=1e-5): 
        ...
        for k in range(self.nb):
            ...
            v = (1-beta)*u+beta*(    0.2*(run_model(t/255,denoisor_sigma) * 255) + 0.8*t ) 
            tt = torch.real(deblur.ifftn(abs_k*deblur.fftn(v) - fft_kH * deblur.fftn(f)))
            t = v - lamb*tt
            # t = torch.clamp(t, min = -0., max =255.)
            t = t.type(torch.cuda.FloatTensor)
            w = (run_model(t/255,denoisor_sigma) * 255)
            u = (1-alpha)*u+alpha*(  0.2*w + 0.8*t )
...
def search_args():
    ...
    # kevin's 8 kernels, CBSD68, 12.75 noise. [1.5,1.6,1.7,1.8,1.9], nb= 100.
    # kernel 01, 
    search_range[0.07] = [1.6] # 27.3882, 0.7608
    # kernel 02, 
    search_range[0.07] = [1.8] # 27.1216, 0.7516
    # kernel 03, 
    search_range[0.07] = [1.9] # 27.5615, 0.7655
    # kernel 04, 
    search_range[0.07] = [1.8] # 26.8647, 0.7398
    # kernel 05,
    search_range[0.07] = [1.8] # 28.6063, 0.8102
    # kernel 06,
    search_range[0.07] = [1.7] # 28.3225, 0.8004
    # kernel 07,
    search_range[0.07] = [1.9] # 27.6020, 0.7741
    # kernel 08,
    search_range[0.07] = [1.9] # 27.1669, 0.7570

    kernel_fp = '/home/dlwei/Documents/pnp_jacobian/kernels/kevin_8.png'
    # kevin's 8 kernels, CBSD68, 17.85 noise. [1.6,1.7,1.8,1.9,2.0], nb= 100.
    # kernel 01, 
    search_range[0.1] = [1.8] # 26.4703, 0.7257
    # kernel 02, 
    search_range[0.1] = [1.9] # 26.2339, 0.7136
    # kernel 03, 
    search_range[0.1] = [2.0] # 26.7559, 0.7317
    # kernel 04, 
    search_range[0.1] = [1.9] # 25.9580, 0.7006
    # kernel 05,
    search_range[0.1] = [1.9] # 27.6647, 0.7753
    # kernel 06,
    search_range[0.1] = [1.8] # 27.3389, 0.7628
    # kernel 07,
    search_range[0.1] = [2.0] # 26.7251, 0.7386
    # kernel 08,
    search_range[0.1] = [1.9] # 26.2802, 0.7177
...
## PnPI-FBS 0037.png, kernel 2. output w. a = 0.05, b = 0.05, nb = 200
# plot_psnr(0.04, 0.7, 12.75) # 24.74
# plot_psnr(0.1, 2.2, 12.75) # 24.48
# plot_psnr(0.08, 2.1, 12.75) # 24.93
# plot_psnr(0.07, 1.7, 12.75) # 24.91, nb = 100, chosen !!!!!!!!!
# plot_psnr(0.1, 1.8, 17.85) # 24.01, nb = 100, chosen !!!!!!!!!
